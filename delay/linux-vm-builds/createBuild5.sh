...
if grep -q Ubuntu /proc/version; then
  ...
  cp $VERSION-Linux.tar.gz $DEST_DIR/$DIR/${VERSION}${SUFFIX}-Linux-$FILE-ubuntu.tar.gz
else
  echo ...copy rpm
  ...
  cp $VERSION-Linux.tar.gz $DEST_DIR/$DIR/${VERSION}${SUFFIX}-Linux-$FILE-centos.tar.gz
fi
