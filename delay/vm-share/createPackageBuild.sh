...
ENABLED_PACKAGES="-DENABLE_LAYOUT=ON -DENABLE_ARRAYS=ON  -DENABLE_MULTI=ON -DENABLE_RENDER=ON -DENABLE_COMP=ON -DENABLE_FBC=ON -DENABLE_GROUPS=ON -DENABLE_REQUIREDELEMENTS=ON -DENABLE_SPATIAL=ON -DENABLE_DYN=ON -DENABLE_QUAL=ON -DENABLE_DISTRIB=ON -DENABLE_L3V2EXTENDEDMATH=ON "
INSTALL_PREFIX=/usr
SUFFIX=-packages
